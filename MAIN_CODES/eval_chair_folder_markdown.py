#!/usr/bin/env python3
"""
eval_chair_folder_markdown_v4.py
...
SCRIPT_VERSION = "2026-05-25-v4-seed-aggregate-latency-default"
DEFAULT_MD_METHODS = ["Greedy", "Beam Search", "DoLA", "OPERA", "VCD", "RVCD"]
...
RUN_PREFIX_RE = re.compile(
    r"^(?P<method>greedy|beam(?:_search)?|beamsearch|dola|opera|vcd|rvcd)[_-](?P<stamp>20\d{6,12})(?:[_-].*)?$",
    re.IGNORECASE,
)
...
def method_from_exact_component(token: str) -> Optional[str]:
    ...
    if norm in {"not_rvcd", "not_rvcd_llava", "non_rvcd", "non_rvcd_llava"}:
        return None
    if norm == "rvcd":
        return None
...
# python eval_chair_folder_markdown.py \
