...
# Hard-coded union of COCO image_ids that had >=2 negative objects
# across seed 42, 43, 44 runs (1000 samples each).
MULTI_PREDEFINED_IMAGE_IDS = [
    589, 6854, 8583, 8603, 12667, 13789, 14773, 15440, 18957, 19132, 19333, 23369, 26363, 27326, 27717, 31521, 35795, 35940, 35963, 37003, 37638, 47524, 48692, 50637, 51028, 51079, 54133, 54533, 64859, 66817, 67315, 76138, 78288, 79969, 86008, 86582, 88560, 93435, 93736, 96832, 98350, 103863, 108253, 113897, 114119, 115870, 117170, 120070, 120961, 123321, 124236, 124766, 128920, 130909, 131969, 132001, 132067, 134133, 134815, 137573, 138054, 143927, 146738, 153445, 154213, 154854, 159736, 169562, 173704, 177529, 180925, 183332, 188416, 189966, 190391, 192079, 196462, 197681, 201899, 201925, 204661, 208340, 208708, 209347, 210032, 218811, 220118, 223699, 224724, 226128, 227227, 232598, 233140, 236148, 241525, 245102, 246626, 248400, 253624, 254143, 254277, 257407, 260486, 265552, 265707, 266627, 268059, 268992, 275117, 278463, 278771, 279085, 281609, 286055, 290532, 291380, 295303, 296907, 299023, 299116, 300994, 301827, 302038, 302219, 303418, 306173, 307342, 307614, 309424, 310663, 310735, 311385, 312103, 313562, 314388, 317798, 318219, 321118, 323119, 323186, 325208, 325992, 326462, 328780, 330405, 336628, 337502, 338905, 339344, 339787, 342380, 351975, 355228, 357385, 357941, 360611, 362306, 362327, 369082, 369404, 369887, 370678, 371117, 376307, 376342, 378139, 378614, 380348, 384111, 384635, 385103, 386210, 392892, 394131, 394478, 394715, 396461, 397658, 397903, 401040, 401092, 404484, 404871, 405998, 407960, 408736, 410380, 415646, 415748, 419723, 424247, 425390, 429807, 430359, 436814, 438737, 439339, 439630, 446462, 451120, 453009, 453824, 455225, 455275, 456261, 456969, 456991, 460156, 461092, 463222, 464622, 465007, 465552, 465718, 468233, 468925, 469609, 473215, 474188, 474344, 480551, 483374, 488201, 488823, 489896, 490620, 493254, 494439, 494629, 494863, 497593, 500878, 503663, 503772, 506052, 509468, 510657, 514016, 514294, 514773, 523526, 525903, 527578, 529968, 530054, 530265, 530975, 533166, 534377, 534801, 535313, 536403, 538586, 542771, 542782, 542939, 546717, 547639, 549410, 553576, 555538, 563730, 564868, 565326, 569801, 570465, 573622, 575081, 575834, 575867, 575970, 577321, 580540
]


def resolve_sampled_img_ids(all_img_ids, requested_num_samples, seed_arg):
    """Return sampled image ids based on seed mode.

    - regular mode: random.sample(all_img_ids, requested_num_samples)
    - multi_predefined mode: use the hard-coded union subset above
    """
    all_img_ids_set = set(int(x) for x in all_img_ids)
    seed_arg = str(seed_arg)

    if seed_arg == "multi_predefined":
        sampled = [int(x) for x in MULTI_PREDEFINED_IMAGE_IDS if int(x) in all_img_ids_set]
        sampled = sorted(dict.fromkeys(sampled))
        if len(sampled) == 0:
            sys.exit("Error: MULTI_PREDEFINED_IMAGE_IDS is empty or none of them exist in the current dataset.")
        print(f"[Sampling] seed=multi_predefined -> using hard-coded union subset with {len(sampled)} images (negative object count >= 2).")
        if requested_num_samples != len(sampled):
            print(f"[Sampling] --num_samples={requested_num_samples} is ignored in multi_predefined mode. Actual sample count: {len(sampled)}")
        return sampled

    try:
        requested_num_samples = int(requested_num_samples)
        if requested_num_samples <= 0:
            raise ValueError
    except Exception:
        sys.exit(f"Error: --num_samples must be a positive integer. Got: {requested_num_samples}")

    if requested_num_samples > len(all_img_ids):
        sys.exit(f"Error: requested_num_samples={requested_num_samples} exceeds dataset size={len(all_img_ids)}")

    return random.sample(all_img_ids, requested_num_samples)
...
parser.add_argument("--seed", type=str, default="42", help="Random seed integer, or 'multi_predefined' to run the hard-coded union subset with >=2 negative objects.")
...
if seed_arg == "multi_predefined":
    seed = "multi_predefined"
    seed_for_setup = 42
    is_multi_predefined_seed = True
else:
    try:
        seed = int(seed_arg)
        seed_for_setup = seed
        is_multi_predefined_seed = False
    except Exception:
        sys.exit(f"Error: --seed must be an integer or 'multi_predefined'. Got: {seed_arg}")
...
######################################
setup_seeds(cfg, seed_for_setup)
...
sampled_img_ids = resolve_sampled_img_ids(img_ids, num_samples, seed_arg)
num_samples = len(sampled_img_ids)
...
global_all_info = {
    ...
    'seed' : seed,
    'seed_arg' : seed_arg,
    'is_multi_predefined_seed' : is_multi_predefined_seed,
    'multi_predefined_image_ids' : MULTI_PREDEFINED_IMAGE_IDS if is_multi_predefined_seed else [],
...
# multi_predefined mode example
# CUDA_VISIBLE_DEVICES=0 \
# python ervcd_generation_chair_bleu_optimized_gridprobe_multi_predefined.py \
#   --model llava-1.5 \
#   --data_path /home/jihoon/jihoon/DATASETS/coco2014/val2014 \
#   --ref_folder_path DB_single_concept_images_flux_generated/generated_images \
#   --chair_cache_path eval/CHAIR_CACHE/chair.pkl \
#   --seed multi_predefined \
#   --gpu-id 0 \
#   --output_dir ./generated_captions_multi_predefined/ \
#   --rvcd_alpha 1 \
#   --rvcd_beta 0 \
