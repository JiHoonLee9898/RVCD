#!/usr/bin/env bash
# setup_rvcd_reliable_a100_blackwell_fixed_v3.sh
#
...
# Usage:
#   cd /path/to/RVCD
#   bash setup_rvcd_reliable_a100_blackwell_fixed_v3.sh
#
# Useful options:
#   INSTALL_COCO=0 bash setup_rvcd_reliable_a100_blackwell_fixed_v3.sh
#   INSTALL_WEIGHTS=0 bash setup_rvcd_reliable_a100_blackwell_fixed_v3.sh
#   INSTALL_PATTERNLITE=0 bash setup_rvcd_reliable_a100_blackwell_fixed_v3.sh
#   STRICT_BNB=1 bash setup_rvcd_reliable_a100_blackwell_fixed_v3.sh
#   RVCD_ROOT=/home/jihoon/jihoon/RVCD bash setup_rvcd_reliable_a100_blackwell_fixed_v3.sh
#
...
install_cuda_build_tools_after_deps() {
  ...
  # Do NOT install cuda-thrust here.
  # On recent NVIDIA conda labels, "cuda-thrust" may not exist as a standalone
  # package. Thrust/CUB/libcu++ headers are now usually provided through CCCL
  # packages, or already pulled in by CUDA dev packages. Requiring cuda-thrust
  # makes the setup fail before GroundingDINO even gets a chance to build.
  run_logged "conda_cuda_build_tools_core.log" conda install -y \
    -c "${cuda_label}" \
    -c nvidia \
    "cuda-nvcc>=${TARGET_CUDA},<${cuda_upper}" \
    "cuda-cudart>=${TARGET_CUDA},<${cuda_upper}" \
    "cuda-cudart-dev>=${TARGET_CUDA},<${cuda_upper}" \
    "cuda-libraries-dev>=${TARGET_CUDA},<${cuda_upper}"

  # Optional: try to install CCCL, which is the modern home of Thrust/CUB/libcu++.
  # Keep this non-fatal because some CUDA labels do not expose the exact package name.
  if ! run_logged_allow_fail "conda_cuda_cccl_optional.log" conda install -y \
    -c "${cuda_label}" \
    -c nvidia \
    "cuda-cccl>=${TARGET_CUDA},<${cuda_upper}"; then
    warn "Optional cuda-cccl package was not available. Continuing without it."
  fi
  ...
  if ! nvcc --version | grep -q "release ${TARGET_CUDA}"; then
    conda list | grep -E 'cuda|cublas|cusolver|cusparse|cudart|cccl|thrust' || true
  ...
  # Required CUDA headers for GroundingDINO/PyTorch CUDA extension builds.
  local missing=0
  for hdr in \
    "${CONDA_PREFIX}/include/cuda_runtime_api.h" \
    "${CONDA_PREFIX}/include/cusparse.h" \
    "${CONDA_PREFIX}/include/cublas_v2.h" \
    "${CONDA_PREFIX}/include/cusolverDn.h"
  do
    if [ -f "${hdr}" ]; then
      echo "Header OK: ${hdr}"
    else
  ...
  if [ "${missing}" -ne 0 ]; then
    conda list | grep -E 'cuda|cublas|cusolver|cusparse|cudart|cccl|thrust' || true
    die "Required CUDA headers are missing."
  fi

  # Thrust header is useful, but do not fail here because not all CUDA conda
  # labels expose it at the same path/package. If GroundingDINO actually needs
  # it and it is absent, the build step will produce a precise compiler error.
  if [ -f "${CONDA_PREFIX}/include/thrust/complex.h" ]; then
    echo "Header OK: ${CONDA_PREFIX}/include/thrust/complex.h"
  else
    warn "Optional thrust header not found at ${CONDA_PREFIX}/include/thrust/complex.h. Continuing."
  fi
}
