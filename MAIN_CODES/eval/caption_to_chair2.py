...
# directory = args.caption_path #수정
# caption_path = args.caption_path #수정
directory = args.generated_caption_path
caption_path = args.gt_caption_path
...
annotation_file_path = caption_path
...
for file_name in caption_files:
    ...
    with open(file_path, "r") as f:
        lines = f.readlines()
        print(f"Number of lines read: {len(lines)}")  # 디버깅 출력
        ...
        print(f"Number of items in loaded_json: {len(loaded_json)}")  # 디버깅 출력

    # 중복 제거 전 크기 확인
    print(f"Before deduplication: {len(loaded_json)}")
    ...
    loaded_json = unique_json

    # 중복 제거 후 크기 확인
    print(f"After deduplication: {len(loaded_json)}")
    ...
    for i in range(len(loaded_json)):
        for j in range(i + 1, len(loaded_json)):
            if loaded_json[i]["image_id"] == loaded_json[j]["image_id"]:
                loaded_json.pop(j)
                break
        # minigpt4의 경우 이상캡션이 가끔 나온다. [가 포함됨. 그때 멈추게 한 코드였음.일단안쓴다
    ...
    output_file_name = file_name.replace("_generated_captions.jsonl", "_chair.json")
